...
struct SessionContextConfig {
    ...
    int     session_open_utc_min{420}; // 10:00 MSK = 07:00 UTC
};

class SessionContext {
public:
    enum class Bias { Long, Short, None };
    using Config = SessionContextConfig;
    ...
    void finalize_orb(double current_price) noexcept {
        ...
        if (orb_high_ < -1e8) { bias_ = Bias::None; return; }
...
private:
    ...
    void update_nr7() noexcept {
        ...
        for (int i = 1; i < cfg_.nr7_period; ++i) {
            ...
            if (daily_ranges_[idx] <= last_range) { nr7_confirmed_ = false; return; }
        }
        nr7_confirmed_ = true;
    }
