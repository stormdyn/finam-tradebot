...
// ── Инструменты ──────────────────────────────────────────────────────────────────────────
//
// Тикеры подтверждены через GetAsset (connectivity_check):
//   SiM6@RTSX, RIM6@RTSX, GDM6@RTSX, MXM6@RTSX, BRM6@RTSX
// Формат тикера: {ROOT}{LETTER}{Y}@RTSX, Y = последняя цифра года
// Чтобы добавить больше инструментов — сначала проверь через connectivity_check.
struct SymbolConfig {
    std::string  ticker;
    double       tick_size;
    int32_t      base_qty;
